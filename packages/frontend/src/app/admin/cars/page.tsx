...
export default function AdminCarsPage() {
    ...
    const handleAddCar = async (e: FormEvent<HTMLFormElement>) => {
        ...
        if (uploadError) {
            ...
            setFormMessage({
                type: 'error',
                text: `Ошибка загрузки изображения: ${uploadError.message}`,
            });
        ...
        if (!imageUrl) {
            setFormMessage({
                type: 'error',
                text: 'Не удалось получить публичный URL для изображения.',
            });
            setIsSubmitting(false);
        ...
        if (insertError) {
            ...
            setFormMessage({
                type: 'error',
                text: `Ошибка добавления автомобиля в базу: ${insertError.message}`,
            });
            await supabase.storage.from('cars').remove([filePath]);
        } else {
            setFormMessage({
                type: 'success',
                text: `Автомобиль "${newCar.name}" успешно добавлен!`,
            });
            setNewCar(initialNewCarState);
            setPreview(null);
            if (document.getElementById('carImageFile')) {
                (
                    document.getElementById('carImageFile') as HTMLInputElement
                ).value = '';
            }
            fetchCars();
    ...
    const handleDeleteCar = async (carToDelete: Car) => {
        if (
            !window.confirm(
                `Вы уверены, что хотите удалить автомобиль "${carToDelete.name}"? Это действие необратимо.`,
            )
        ) {
            return;
        }
        ...
        try {
            ...
            imagePath = url.pathname.substring(
                url.pathname.indexOf('/cars/') + '/cars/'.length,
            );
        } catch (err) {
            console.error(
                'Invalid image URL, cannot extract path for deletion:',
                carToDelete.image_url,
                err,
            );
            alert(
                'Не удалось определить путь к файлу изображения для удаления. Удаление из базы данных будет продолжено.',
            );
        }
        ...
        if (imagePath) {
            ...
            if (storageError) {
                ...
                alert(
                    `Ошибка при удалении изображения из хранилища: ${storageError.message}. Запись из базы данных будет удалена.`,
                );
            }
        }
        ...
        if (dbError) {
            ...
            alert(
                `Ошибка при удалении автомобиля из базы данных: ${dbError.message}`,
            );
        } else {
                            ...
                            >
                                ...
                                <div className="grid grid-cols-1 md:grid-cols-2 gap-x-6 gap-y-5">
                                    <FormInputField
                                        ...
                                        placeholder="Mercedes-Benz S580"
                                        value={newCar.name}
                                        onChange={(e) =>
                                            setNewCar({
                                                ...newCar,
                                    ...
                                    <FormInputField
                                        id="class"
                                        label="Класс"
                                        placeholder="Люкс, Премиум, Бизнес"
                                        value={newCar.class}
                                        onChange={(e) =>
                                            setNewCar({
                                                ...newCar,
                                ...
                                {formMessage && (
                                    <div
                                        className={`flex items-center gap-2 p-3 rounded-md text-sm mt-1 ${formMessage.type === 'success' ? 'bg-green-900/30 border border-green-700/50 text-green-300' : ''} ${formMessage.type === 'error' ? 'bg-red-900/30 border border-red-700/50 text-red-300' : ''}`}
                                    >
                        ...
                        <div>
                            <div className="flex justify-between items-center mb-4">
                                <h3 className="text-xl sm:text-2xl font-semibold text-white">
                                    Список автомобилей ({cars.length})
                                </h3>
                                <button
                                    onClick={fetchCars}
                                    disabled={isLoading}
                            ...
                            {isLoading && cars.length === 0 ? (
                                ...
                                <div className="text-center py-16 px-6 bg-neutral-800/30 border border-neutral-700 rounded-lg">
                                    ...
                                    <p className="text-sm text-neutral-400">
                                        Добавьте свой первый автомобиль с
                                        помощью формы выше.
                                    </p>
                                </div>
                            ) : (
                                            ...
                                            <div className="flex flex-col sm:flex-row items-center gap-4 sm:gap-5 p-4 sm:p-5">
                                                <div className="w-full sm:w-40 h-32 sm:h-28 flex-shrink-0 relative rounded-lg overflow-hidden bg-neutral-700/50">
                                                    <Image
                                                        ...
                                                        onError={(imgEvent) => {
                                                            (
                                                                imgEvent.target as HTMLImageElement
                                                            ).src =
                                                                '/cars/placeholder-car.png';
                                                        }}
                                                    />
