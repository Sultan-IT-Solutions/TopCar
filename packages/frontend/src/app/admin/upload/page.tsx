...
const PlaceholderAdminUploadPanel = () => {
    ...
    const handleUpload = async () => {
        ...
        setMessage({
            text: `Файл "${selectedFile.name}" успешно загружен.`,
            type: 'success',
        });
        setSelectedFile(null);
        if (document.getElementById('fileUploadInput')) {
            (
                document.getElementById('fileUploadInput') as HTMLInputElement
            ).value = '';
        }
        setUploading(false);
        ...
        <div className="space-y-8">
            ...
            <div className="bg-neutral-800/50 border border-neutral-700 rounded-xl p-6 shadow-lg">
                ...
                <div className="space-y-4">
                    ...
                    {message && (
                        <div
                            className={`flex items-center gap-2 p-2.5 rounded-md text-xs
                ...
                <div className="bg-neutral-800/50 border border-neutral-700 rounded-xl p-6 shadow-lg">
                    ...
                    <ul className="space-y-3 max-h-96 overflow-y-auto scrollbar-thin scrollbar-thumb-neutral-700 scrollbar-track-neutral-800">
                        {uploadedFiles.map((file, index) => (
                            <li
                                key={index}
                                className="flex items-center justify-between p-3 bg-neutral-900/70 rounded-md border border-neutral-600/50"
                            >
                                <div className="flex items-center gap-3 overflow-hidden">
                                    {file.type === 'image' ? (
                                        <PhotoIcon className="h-5 w-5 text-[#d4af37] flex-shrink-0" />
                                    ) : (
                                        <DocumentTextIcon className="h-5 w-5 text-neutral-400 flex-shrink-0" />
                                    )}
                                    <a
                                        href={file.url}
                                        target="_blank"
                                        rel="noopener noreferrer"
                                        className="text-sm text-neutral-200 hover:text-[#d4af37] truncate"
                                        title={file.name}
                                    >
                                        {file.name}
                                    </a>
    ...
    if (!adminUser) {
        return (
            <div className="min-h-screen bg-neutral-950 flex flex-col items-center justify-center text-white p-8">
                ...
                <Link
                    href="/"
                    className="px-6 py-2 bg-[#d4af37] text-black rounded-md font-medium hover:bg-[#c0982c] transition-colors"
                >
                    На главную
                </Link>
            </div>
                            ...
                            <div className="flex items-center gap-3">
                                ...
                                <div>
                                    ...
                                    <p className="mt-1 text-sm text-neutral-400">
                                        Загрузка и управление медиа-ресурсами
                                        сайта.
                                    </p>
                                </div>
                            </div>
