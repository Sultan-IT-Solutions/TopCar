...
export async function POST(req: NextRequest) {
    try {
        const { userName, userPhone, carName, bookingDetails } =
            await req.json();
        ...
        if (!baseUrl) {
            throw new Error(
                'BITRIX_WEBHOOK_URL is not set in environment variables',
            );
        }
        ...
        if (result.error) {
            ...
            return NextResponse.json(
                { message: `Ошибка CRM: ${result.error_description}` },
                { status: 500 },
            );
        }
        ...
        return NextResponse.json(
            { message: 'Лид успешно создан в Bitrix24', leadId: result.result },
            { status: 200 },
        );
    } catch (error) {
