...
export default async function CarDetailPage({
    ...
    const dailyPrice =
        car!.prices?.find((p) => !p.with_driver && p.days_from === 24)
            ?.price_per_day ||
        car!.price_per_day ||
        0;
    ...
    const features = [
        {
            icon: Zap,
            label: 'Мощность',
            value: car!.power ? `${car!.power} л.с.` : 'н/д',
        },
        ...
        { icon: Users, label: 'Кол-во мест', value: car!.seats || 'н/д' },
        {
            icon: ClockIcon,
            label: 'Разгон до 100',
            value: car!.acceleration ? `${car!.acceleration} сек` : 'н/д',
        },
    ];
        ...
        <AnimatedPageWrapper>
            ...
            <main className="min-h-screen bg-neutral-950 text-white font-sans pt-24">
                ...
                <nav
                    ...
                    className="px-4 sm:px-6 max-w-6xl mx-auto mb-4 text-sm text-neutral-400"
                >
                    <ol className="flex flex-wrap items-center gap-2">
                        ...
                        <li>
                            <Link href="/autopark" className="hover:text-white">
                                Автопарк
                            </Link>
                        </li>
                        <li className="opacity-60">/</li>
                        <li
                            aria-current="page"
                            className="text-white truncate max-w-[60vw] sm:max-w-none"
                        >
                            {car!.name}
                        </li>
                    </ol>
                                    ...
                                    <div className="grid grid-cols-3 gap-4">
                                        {car!.gallery_images.map(
                                            (img, index) => (
                                                <div
                                                    key={index}
                                                    className="aspect-video relative rounded-md overflow-hidden bg-neutral-800"
                                                >
                                                    <Image
                                                        src={img}
