...
export default function CalculatorModal({
    ...
    useEffect(() => {
        if (isOpen) {
            const fetchCars = async () => {
                ...
                if (error)
                    console.error(
                        'Ошибка загрузки автомобилей с ценами:',
                        error,
                    );
    ...
    const selectedCar = useMemo(
        () => carsData.find((c) => c.id === Number(selectedCarId)) || null,
        [selectedCarId, carsData],
    );

    const { availableDurations, totalPrice, selectedPriceInfo } =
        useMemo(() => {
            ...
            const relevantPrices = selectedCar.prices
                .filter((p) => p.with_driver === isWithDriver)
                .sort((a, b) => a.days_from - b.days_from);

            const durationOptions: DurationOption[] = relevantPrices.map(
                (p) => {
                    const durationLabel =
                        p.days_from === p.days_to
                            ? `${p.days_from}ч`
                            : `${p.days_from}-${p.days_to} дней`;
                    return {
                        label: durationLabel,
    ...
    const handleBookingSubmit = () => {
        ...
        const bookingDetails: BookingDetails = {
            serviceType:
                serviceType === 'withDriver' ? 'С водителем' : 'Без водителя',
                                    ...
                                    <div className="mt-6 space-y-6">
                                        <div>
                                            ...
                                            <div className="relative">
                                                <select
                                                    id="carSelectCalc"
                                                    className="w-full pl-3 pr-10 py-3 text-base text-white bg-neutral-800 border border-neutral-600 rounded-lg focus:outline-none focus:ring-2 focus:ring-[#d4af37] focus:border-[#d4af37] appearance-none"
                                                    value={selectedCarId}
                                            ...
                                            <div>
                                                ...
                                                <div className="grid grid-cols-2 rounded-lg bg-neutral-800 p-1">
                                                    <button
                                                        ...
                                                        className={`px-4 py-2 text-sm font-medium rounded-md transition-colors duration-200 ${serviceType === 'withoutDriver' ? 'bg-[#d4af37] text-black shadow' : 'text-neutral-300 hover:bg-neutral-700'}`}
                                                    >
                                                        Без водителя
                                                    </button>
                                                    <button
                                                        ...
                                                        className={`px-4 py-2 text-sm font-medium rounded-md transition-colors duration-200 ${serviceType === 'withDriver' ? 'bg-[#d4af37] text-black shadow' : 'text-neutral-300 hover:bg-neutral-700'}`}
                                                    >
                                                        С водителем
                                                    </button>
                                                </div>
                                        ...
                                        <button
                                            ...
                                            className="w-full mt-6 px-8 py-3 bg-[#d4af37] text-black rounded-lg text-base font-semibold hover:bg-[#c0982c] transition-colors duration-300 focus:outline-none focus:ring-4 focus:ring-[#d4af37]/50 disabled:opacity-50 disabled:cursor-not-allowed"
                                        >
