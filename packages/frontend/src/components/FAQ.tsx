...
export default function FAQ() {
    ...
    return (
        <section
            id="faq"
            className="py-24 sm:py-32 px-4 sm:px-6 bg-neutral-950"
        >
            <script
                    ...
                    {faqData.map((item, idx) => (
                        <FadeInWhenVisible key={idx} delay={idx * 0.05}>
                            <details className="group bg-neutral-900 border border-neutral-700/70 rounded-xl shadow-lg transition-all duration-300 ease-in-out hover:border-[#d4af37]/50">
                                <summary className="flex justify-between items-center p-6 cursor-pointer list-none group-hover:bg-neutral-800/50 rounded-t-xl transition-colors">
