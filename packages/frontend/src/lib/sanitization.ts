 ...
 */
export function sanitizeHtml(dirty: string, config: DOMPurify.Config = defaultConfig): string {
    ...
    if (obj.hasOwnProperty(key)) {
      ...
      if (typeof value === 'string') {
        ...
        result[key] = value.map((item: any) =>
          typeof item === 'string' ? sanitizeHtml(item, config) :
          typeof item === 'object' ? sanitizeObject(item, config) :
          item
        );
      } else {
        result[key] = value;
